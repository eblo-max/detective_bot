...
class EnergyManager:
    ...
    async def calculate_current_energy(self, user: User) -> int:
        ...
        time_passed = datetime.utcnow() - user.last_energy_update
    ...
    async def can_perform_action(
        ...
        if current_energy < cost:
            time_to_restore = self.get_next_full_energy_time(user)
            return (
                False,
    ...
    async def consume_energy(self, user: User, action_type: ActionType) -> bool:
        ...
        can_perform, error_message = await self.can_perform_action(user, action_type)
        if not can_perform:
            return False
...
class EnergyPlanner:
    ...
    def get_optimal_action_sequence(
        ...
        for action in sorted_actions:
            can_perform, _ = energy_manager.can_perform_action(user, action)
            if can_perform:
