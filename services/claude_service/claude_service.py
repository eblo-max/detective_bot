...
class SemanticCache:
    ...
    def set(self, query: str, response: str) -> None:
        ...
        self.cache[query] = (response, datetime.now(timezone.utc))
...
class TokenOptimizer:
    ...
    def reuse_previous_response(
        ...
        Args:
            ...
            similarity_threshold: Порог сходства для повторного использования (0.0 - 1.0)

        Returns:
            Optional[str]: Части предыдущего ответа, которые можно повторно использовать,
                          или None, если подходящих частей не найдено
        """
        # Разбиваем предыдущий ответ на предложения
        sentences = [s.strip() for s in previous_response.split(".") if s.strip()]

        # Если предыдущий ответ пустой или не содержит предложений
        if not sentences:
            return None

        # Используем библиотеку sentence_transformers для получения эмбеддингов
        try:
            from sentence_transformers import SentenceTransformer

            model = SentenceTransformer("all-MiniLM-L6-v2")

            # Получаем эмбеддинги для текущего промпта и предложений
            prompt_embedding = model.encode(current_prompt, convert_to_tensor=True)
            sentence_embeddings = model.encode(sentences, convert_to_tensor=True)

            # Вычисляем косинусное сходство между промптом и каждым предложением
            from torch.nn.functional import cosine_similarity

            similarities = [
                cosine_similarity(prompt_embedding, sent_emb, dim=0).item()
                for sent_emb in sentence_embeddings
            ]

            # Выбираем предложения, превышающие порог сходства
            reusable_sentences = [
                sent
            ...
            # Если нашли подходящие предложения, объединяем их
            if reusable_sentences:
                return ". ".join(reusable_sentences) + "."
...
class ClaudeService:
    ...
    async def _handle_response(self, response: Any) -> None:
        ...
        if hasattr(response, "error"):
            ...
            raise ClaudeAPIError(f"API вернул ошибку: {response.error}")

        # Обработка успешного ответа
        content = response.content[0].text if response.content else ""

        # Сохраняем в кэш, если есть ключ кэширования
        if hasattr(self, "_current_cache_key") and self._current_cache_key:
            await self.semantic_cache.set(self._current_cache_key, content)

        # Очищаем текущий ключ кэша
    ...
    async def generate_investigation_step(
        ...
        if self.request_queue and self.request_queue[-1].response:
            return self.request_queue[-1].response.content[0].text
    ...
    async def generate_psychological_profile(self, suspect_data: Dict[str, Any]) -> str:
        ...
        return "Извините, не удалось сгенерировать психологический профиль. Пожалуйста, попробуйте еще раз."
    ...
    async def generate_news(
        ...
        if self.request_queue and self.request_queue[-1].response:
            try:
                return json.loads(self.request_queue[-1].response.content[0].text)
            except json.JSONDecodeError:
                logger.error("Не удалось распарсить JSON ответ от API")
                return []

        return [
            {
                "error": "Не удалось сгенерировать новости. Пожалуйста, попробуйте еще раз."
            }
        ]
