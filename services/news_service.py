...
class NewsService:
            ...
            user.stats["skill_bonuses"].append(
                {
                    ...
                    "amount": bonus["amount"],
                    "expires_at": (
                        datetime.now(timezone.utc) + timedelta(hours=bonus["duration"])
                    ).isoformat(),
                }
            )
            ...
            user.stats["special_conditions"].append(
                {
                    ...
                    "expires_at": (
                        ...
                        + timedelta(hours=condition["duration"])
                    ).isoformat(),
                }
            )
    ...
    async def _get_user_context(self) -> NewsContext:
        """
        Получение контекста пользователя из базы данных.

        Returns:
            NewsContext: Контекст пользователя с информацией о навыках, активных делах и интересах
        """
        try:
            # Получаем данные пользователя из репозитория
            user_data = await self.repository.get_user_data()

            # Получаем активные расследования
            active_cases = await self.repository.get_active_cases()

            # Получаем недавние новости
            recent_news = await self.repository.get_recent_news(limit=5)

            # Формируем контекст
            return NewsContext(
                player_level=user_data.get("level", 1),
                player_skills=user_data.get("skills", {}),
                active_cases=[case.title for case in active_cases],
                recent_news=[news.title for news in recent_news],
                player_interests=user_data.get("interests", []),
            )
        except Exception as e:
            logger.error(f"Ошибка при получении контекста пользователя: {e}")
            # Возвращаем дефолтный контекст в случае ошибки
            return NewsContext(
                player_level=1,
                player_skills={},
                active_cases=[],
