...
ERROR_MESSAGE = "❌ Произошла ошибка при начале расследования.\n" + "Попробуйте позже."
CASE_NOT_FOUND_MESSAGE = "❌ Ошибка: расследование не найдено"

# Константы для шаблонов обработчиков
...
async def handle_main_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if not case_id:
            await query.message.edit_text(CASE_NOT_FOUND_MESSAGE)
            return ConversationHandler.END
...
investigation_handler = ConversationHandler(
    ...
    states={
        States.MAIN_MENU: [
            CallbackQueryHandler(handle_main_action, pattern=ACTION_PATTERN),
            CallbackQueryHandler(select_case, pattern="^case_"),
        ],
        States.EXAMINING_SCENE: [
            CallbackQueryHandler(handle_examination_action, pattern=ACTION_PATTERN)
        ],
        States.INTERVIEWING_WITNESS: [
            CallbackQueryHandler(handle_interrogation_action, pattern=ACTION_PATTERN)
        ],
        States.ANALYZING_EVIDENCE: [
            CallbackQueryHandler(handle_analysis_action, pattern=ACTION_PATTERN)
        ],
        States.MAKING_DEDUCTION: [
            CallbackQueryHandler(handle_deduction_action, pattern=ACTION_PATTERN)
        ],
        States.ANALYZING: [
...
async def show_investigation_status(query: Any, investigation) -> None:
    """Показывает текущий статус расследования."""
    status_text = (
        f"🔍 *Расследование №{investigation.id}*\n\n"
        f"Статус: {investigation.status}\n"
        f"Прогресс: {investigation.progress}%\n"
        f"Улики: {len(investigation.evidence)}/5\n"
        f"Подозреваемые: {len(investigation.suspects)}\n\n"
        f"Текущая локация: {investigation.current_location}"
    )

    await query.message.edit_text(
        status_text,
        parse_mode="Markdown",
        reply_markup=await create_investigation_keyboard(investigation),
    )


async def examine_evidence(
    query: Any, context: CallbackContext, evidence_id: Optional[str]
) -> None:
    """Обработка осмотра улики."""
    try:
        if not evidence_id:
            await query.message.edit_text("❌ Улика не найдена")
            return

        investigation = await investigation_repository.get_active_investigation(
            query.from_user.id
        )
        if not investigation:
            await query.message.edit_text("❌ Активное расследование не найдено")
            return

        result = await claude_service.generate_next_step(
            case_id=investigation.case_id, action_type="examine", target_id=evidence_id
        )

        await query.message.edit_text(
            f"🔍 Результат осмотра улики:\n\n{result}",
            parse_mode="Markdown",
            reply_markup=await create_investigation_keyboard(investigation),
        )

    except Exception as e:
        logger.error(f"Ошибка при осмотре улики: {e}")
        await query.message.edit_text("❌ Произошла ошибка при осмотре улики")


async def interrogate_suspect(
    query: Any, context: CallbackContext, suspect_id: Optional[str]
) -> None:
    """Обработка допроса подозреваемого."""
    try:
        if not suspect_id:
        ...
        logger.error(f"Ошибка при допросе подозреваемого: {e}")
        await query.message.edit_text("❌ Произошла ошибка при допросе подозреваемого")


async def solve_investigation(
    query: Any, context: CallbackContext, investigation_id: Optional[str]
) -> None:
    """Обработка завершения расследования."""
    try:
        ...
        logger.error(f"Ошибка при завершении расследования: {e}")
        await query.message.edit_text(
            "❌ Произошла ошибка при завершении расследования"
        )
