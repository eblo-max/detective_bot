...
from sqlalchemy import select, and_, or_, desc, join
...
class RelationshipRepository(BaseRepository[Relationship]):
    ...
    async def search_relationships(
        ...
        # Поиск по имени пользователя через JOIN с таблицей пользователей
        stmt = (
            select(Relationship)
            .join(User, Relationship.target_id == User.id)
            .where(
                and_(
                    *conditions,
                    or_(
