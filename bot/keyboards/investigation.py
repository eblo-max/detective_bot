...
class InvestigationKeyboards:
    ...
    @staticmethod
    async def create_investigation_keyboard(investigation) -> InlineKeyboardMarkup:
        """Создает клавиатуру для расследования."""
        keyboard = [
            [
                InlineKeyboardButton(
                    "🔍 Осмотреть улики",
                    callback_data=f"investigation_examine_{investigation.id}",
                ),
                InlineKeyboardButton(
                    "👤 Допросить",
                    callback_data=f"investigation_interrogate_{investigation.id}",
                ),
            ],
            [
                InlineKeyboardButton(
                    "🔬 Анализировать",
                    callback_data=f"investigation_analyze_{investigation.id}",
                ),
                InlineKeyboardButton(
                    "🎯 Завершить",
                    callback_data=f"investigation_solve_{investigation.id}",
                ),
            ],
            [InlineKeyboardButton("« Назад", callback_data="investigation_back")],
        ]
        return InlineKeyboardMarkup(keyboard)
